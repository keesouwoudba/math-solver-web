...
export class SolverHomePage extends HTMLElement {
    ...
    dynamicVDOM = [
            {tag: "div", className: "workspace-card", id: "", 
                children: [
                    ...
                    {tag: "textarea", className:"textfield", placeholder: "Enter Formula", id: "formula-input", stateRef: this.inputStateRef}
                ]
            }, 
            {tag: "div", className: "keypad",
                children: [
                    ...
                    {tag: "div", className: "keypad-grid active", id:"keypad-algebraic", 
                        children :[
                            {tag: "button", className: "button-white", id:"operator-add", type: "operator", value: "+"},
                            {tag: "button", className: "button-white", id:"operator-subtract", type: "operator", value: "-"},
                            {tag: "button", className: "button-white", id:"operator-multiply", type: "operator", value: "*"},
                            {tag: "button", className: "button-white", id:"operator-divide", type: "operator", value: "/"}, 
                            {tag: "button", className: "button-white", id:"operator-pow", type: "operator", value: "()**()"},
                            {tag: "button", className: "button-white", id:"function-sqrt", type: "function", value: "sqrt()"},
                            {tag: "button", className: "button-white", id:"function-log", type: "function", value: "log()"},
                            {tag: "button", className: "button-white", id:"function-ln", type: "function", value: "ln()"}

                        ]
                    },
                    {tag: "div", className: "keypad-grid", id: "keypad-trig", 
                        children: [
                            {tag: "button", className: "button-white", id:"function-sin", type: "function", value: "sin()"}, 
                            {tag: "button",className: "button-white", id:"function-cos", type: "function", value: "cos()"},
                            {tag: "button",className: "button-white", id:"function-tan", type: "function", value: "tan()" },
                            {tag: "button",className: "button-white", id:"function-cot", type: "function", value: "cot()"}, 
                            {tag: "button",className: "button-white", id:"function-csc", type: "function", value: "csc()"},
                            {tag: "button",className: "button-white", id:"function-sec", type: "function", value: "sec()"}
                        ]
                    },
                    {tag: "div", className: "helper-btns-container", id: "helper-btns-container",
                        children: [
                            {tag: "button", className: "btn-clear", id: "btn-clear", value: "Clear 🗑️"},
                            {tag: "button", className: "btn-backspace", id: "btn-backspace", value: "⌫"}, 
                            {tag: "button", className: "btn-solve", id: "btn-solve", value: "Solve ✅"}
                        ]
                    },
    ...
    setTextareaValue(value){
        this.inputStateRef.current = value;
        this.inputStateRef.isFocused = true; 
        this.inputStateRef.selectionStart = value.length; 
        this.inputStateRef.selectionEnd = value.length;
    ...
    attachEventListeners(){
            ...
            if (textarea){
                this.root.addEventListener("focusin", (event) => {
                        ...
                        if (target.id !== "formula-input") { return; }  
                        this.inputStateRef.isFocused = true;
            ...
            this.root.addEventListener("input", (event) => {
                ...
                console.log(`SolverHomePage: User input updated to "${this.inputStateRef.current}"`);
            ...
            if (backspaceBtn){
                backspaceBtn.addEventListener("click", () => {
                    ...
                    console.log(`SolverHomePage: Backspace button clicked, user input updated to "${this.inputStateRef.current}"`);
                    this.MyVDOMService.updateDOM("from backspace button click");
                });
            }

            //add event listeners for operator and function buttons
            const operatorAndFunctionButtons = this.root.querySelectorAll(".button-white");
            operatorAndFunctionButtons.forEach((btn) => {
                btn.addEventListener("click", (event) => {
                    ...
                    const textarea = this.root.querySelector("#formula-input");
                    if (textarea){
                        this.inputStateRef.selectionStart = textarea.selectionStart;
                        this.inputStateRef.selectionEnd = textarea.selectionEnd;
                        this.inputStateRef.isFocused = textarea.matches(":focus");
                    }
                    ...
                    if(id.startsWith("operator-")){
                        //insert value of operator button at cursor position
                        ...
                        this.data.inputStateRef.selectionEnd = this.data.inputStateRef.selectionStart;
                        } else {
                            ...
                            const beforeCursor = this.data.inputStateRef.current.slice(0, this.data.inputStateRef.selectionStart);
                            const regex = /([a-zA-Z0-9\)]+)$/;
                            const match  = beforeCursor.match(regex);
                            if(match){
                                const toWrap = match[1];
                                const startIdx = this.data.inputStateRef.selectionStart - toWrap.length;
                                ...
                                this.data.inputStateRef.selectionStart = startIdx + toWrap.length + 4; //to move cursor inside second parentheses
                                this.data.inputStateRef.selectionEnd = this.data.inputStateRef.selectionStart;
                            }
                            else {
                                //means there is nothing to wrap or space, just insert ()**() and put cursor in the middle of first parentheses
                                ...
                                this.data.inputStateRef.selectionStart += 3; //to move cursor in the middle of first parentheses
                                this.data.inputStateRef.selectionEnd = this.data.inputStateRef.selectionStart;
                            }
                        }
                        ...
                        if(match){
                            ...
                            this.data.inputStateRef.current = `${this.data.inputStateRef.current.slice(0, this.data.inputStateRef.selectionStart)} * ${valueToInsert} ${this.data.inputStateRef.current.slice(this.data.inputStateRef.selectionEnd)}`;
                            this.data.inputStateRef.selectionStart += valueToInsert.length + 4; //to move cursor in the middle of parentheses after function name
                            this.data.inputStateRef.selectionEnd = this.data.inputStateRef.selectionStart;
                        } else {
                            ...
                            this.data.inputStateRef.selectionStart += valueToInsert.indexOf("()") + 1;
                            this.data.inputStateRef.selectionEnd = this.data.inputStateRef.selectionStart;
                        }

                    }
                    this.MyVDOMService.updateDOM(`from operator/function button click, id: ${id}`);
                });
            });
